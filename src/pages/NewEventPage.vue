...
watchEffect(() => {
  if (isEditing.value) {
    ...
    const existingEvent = eventsFromLocalStorage.find((ev) => ev.id === route.params.id)
    if (existingEvent) {
...
const saveEvent = (eventData) => {
  ...
  if (isEditing.value) { // se estiver editando...
    const indexOfExistingEvent = eventsFromLocalStorage.findIndex((ev) => ev.id === route.params.id)
